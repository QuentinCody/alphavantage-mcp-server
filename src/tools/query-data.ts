...
export function registerQueryData(server: McpServer, env?: QueryEnv) {
    ...
    server.registerTool(
        "av_query_data",
        {
            title: "Query Staged Alpha Vantage Data",
            description:
                "Query staged Alpha Vantage data using SQL. Use this when responses are too large and have been staged with a data_access_id.",
            inputSchema: {
