...
console.log(`${BLUE}🧪 Alpha Vantage Structured Content Regression Tests${RESET}`);

const indexContent = readFile('src/index.ts');
assertContains('src/index.ts', indexContent, 'AvDataDO', 'index.ts exports AvDataDO');
...
assertContains('src/tools/code-mode.ts', codeModeContent, 'avCatalog', 'code-mode.ts imports avCatalog');
...
assertContains('src/spec/catalog.ts', catalogContent, 'Alpha Vantage', 'catalog.ts names the API');
...
console.log(`\n${GREEN}✅ Alpha Vantage structured content regression tests passed.${RESET}`);
